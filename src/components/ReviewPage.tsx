      ...
      parsedData.blocks.forEach((block: any) => {
        if (block.type === "header" && sectionTitles.includes(block.data.text)) {
          ...
          currentSection = { 
            ...
            content: JSON.stringify({ blocks: [] }) // 👈 Store as Editor.js JSON format
          };
        } else if (block.type === "paragraph" && currentSection) {
          // Append paragraph content
          let parsedContent = JSON.parse(currentSection.content);
          parsedContent.blocks.push(block);
          currentSection.content = JSON.stringify(parsedContent);
        }
      });
